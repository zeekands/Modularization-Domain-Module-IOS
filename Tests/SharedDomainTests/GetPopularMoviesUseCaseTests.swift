...
final class GetPopularMoviesUseCaseTests: XCTestCase {
  
  var mockRepository: MockMovieRepository!
  var useCase: GetPopularMovies!
  ...
  func testExecute_success() async throws {
    let movies = [
      MovieEntity(id: 1, title: "Test Movie 1"),
      MovieEntity(id: 2, title: "Test Movie 2")
    ]
    ...
    XCTAssertEqual(fetchedMovies.first?.title, "Test Movie 1")
  }
  
  @MainActor
  func testExecute_failure() async {
    let expectedError = MockError.notImplemented
    mockRepository.getPopularMoviesResult = .failure(expectedError)
    do {
